...
c_presto="trino --server etl01:18080 --catalog hive"
c_all_ql="trino --server etl01:18080 --catalog hive"
c_sql="java -jar /opt/infalog/bin/package/jdbcsql.jar -H -f MYSQL -h inidb -p 1521 -m oracle -U stg01 -P stg01pwd -d ini"
c_sql3="/opt/infalog/bin/jdbc2console.sh -U jdbc:oracle:thin:@inidb:1521/ini -u stg01 -p stg01pwd"
self=$(basename $0)

rds() {
	echo "$1" | redis-cli -h etl01 -p 6379 -a Hw5YUBAi -n 8
}
...
rfg() {
	if [ "x$2" != "x" ]; then
		...
		wtin) ##wtin :等待标志出现
			while [ $(rds "sismember 1 $2") == 0 ]; do
				echo "$(date +'%F %T'):标志[$2]不存在，等待标志出现..."
				sleep 1
			done ;;

		wtout) ##wtout:等待标志消失
			while [ $(rds "sismember 1 $2") == 1 ]; do
				echo "$(date +'%F %T'):标志[$2]存在，等待标志消失..."
				sleep 1
			done ;;
		esac
	fi
...
tip() {
	...
	#输出文本
	echo -e "--------[<b>${shname}</b>]***${shsts}***[当前时间:$(date +"%F %T")][服务器:$(hostname)][$3]--------\n"
}
...
plan_start() {
	...
	if [ "x$(rfg add plan_start)" == "x1" ]; then
		...
		##获取需要调起的任务
		for line in $(${c_sql3} "select stg01.fn_imp_value('plan_run') from dual"); do
			if [ "x${line}" != "x" ]; then
				IFS='|'
				linev=(${line})
				IFS=' '
...
sp_init() {
	...
	if [ "x$(rfg add sp_init)" == "x1" ]; then
		...
		for sysid in $(${c_sql3} "select stg01.fn_imp_value('etl_end') from dual"); do
			if [ "x${sysid}" != "x" ]; then
				##置数据源采集结束
		...
		if [ $? -eq 0 ]; then
			for line in $(${c_sql3} "select stg01.fn_imp_value('sp_run') from dual"); do
				if [ "x${line}" != "x" ]; then
					##置状态为R
					IFS='|'
					linev=(${line})
...
sp_etl() {
	...
	if [ "x"$(rfg add sp.$1) == "x1" ]; then
		...
		for comt in $(${c_sql3} "select com_id||','||com_kind||','||com_idx from stg01.tb_imp_sp_com where sp_id='$1' and flag='N' order by com_idx"); do
...
ds_etl() {
	...
	if [ "x"$(rds "get ds.${ds_id}") == "x0" -a "x"$(rds "get ds.${ds_id}.bupdate") == "x1" ]; then
		...
		for db_conn in $(${c_sql} "select sou_db_conn from vw_imp_ds2_mid where ds_id='${ds_id}' group by sou_db_conn union all select '${ds_id}' from dual union all select 'hadoop' from dual"); do
			(
				soutab_etl "${db_conn}"
				if [ $? -ne 0 ]; then rds "set ds.${ds_id} 1"; fi
			) &
	...
	if [ "x"$(rds "get ds.${ds_id}") == "x0" ]; then
		...
		for tbl_id in $(cat ${dest_dir}/${ds_id}.txt); do
	...
	for rs in $(rds "keys ds.${ds_id}*"); do
...
soutab_start() {
	if [ "x$(rfg add soutab)" == "x1" ]; then
		...
		dest_dir=$(rds "get path.oths")
		for db_conn in $(${c_sql} "select sou_db_conn from stg01.vw_imp_etl_soutab where kind='etl'"); do
			(
		...
		for kind in {updt_hive,updt_mysql}; do
...
updt_param() {
	...
	##计算日期参数
	for td in {"sysdate","sysdate-1","sysdate+1"}; do
	...
	${c_sql} "select rds from stg01.vw_updt_rds" | while read line; do
...
start_wkf() {
	...
	*)
		## 直接调起命令
		# /opt/infalog/bin/sp_alone.sh $1 $2
		eval $1 $2
		;;
	esac
...
etl_end() {
	...
	${c_sql} "select replace(rds,'set param.','set api.${sysid}') rds from stg01.vw_updt_rds where rds like 'set param.%'" | while read line; do
		echo "更新redis:${line}===>"$(rds "$line")
	done
